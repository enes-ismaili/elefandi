<x-admin-layout>
    ...
    <div class="product-area">
                ...
                <table id="myTable" class="table" style="margin: 25px 0;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{!! ($ad->mimage) ? '<img src="'.asset('photos/ads/'.$ad->mimage).'" height="50">' : '' !!}</td>
                            <td>{{ ($ad->vendor_id) ? $ad->vendor->name : 'Administratori' }}</td>
                            <td>{{ $ad->view }}</td>
                            <td>{{ $ad->click }}</td>
                            <td>{{ Carbon\Carbon::parse($ad->created_at)->format('H:i d.m.Y') }}</td>
                            <td class="action-icons">
                                <a href="{{ route('vendor.ads.single.edit', [$mads->id, $ad->id]) }}" class="action-icon" title="Ndrysho"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                @if(check_permissions('delete_rights'))
                                <span class="deleteModal action-icon"  title="Fshi" onclick="deleteModalF(this)"
                                    data-link="{{ route('vendor.ads.single.delete', [$mads->id, $ad->id]) }}" 
        ...
        <div class="row">
            <div class="col-12">
                <a href="{{ route('vendor.ads.single.add', $mads->id) }}" class="btn btn-primary small tableadd c3"><i class="fas fa-plus"></i> Shto Reklamë</a>
            </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            @elseif($ad->fvaction == 1)
                                <td>{{ $ad->view + $ad->fview }}</td>
                            @elseif($ad->fvaction == 2)
                                <td>{{ ($ad->view && $ad->fview) ? ($ad->view * $ad->fview) : $ad->view }}</td>
                            @endif
                            @if($ad->fcaction == 0)
                                <td>{{ $ad->click }}</td>
                            @elseif($ad->fcaction == 1)
                                <td>{{ $ad->click + $ad->fclick }}</td>
                            @elseif($ad->fcaction == 2)
                                <td>{{ ($ad->click && $ad->fclick) ? ($ad->click * $ad->fclick) : $ad->click }}</td>
                            @endif
                            <td>{{ Carbon\Carbon::parse($ad->created_at)->format('H:i d.m.Y') }}</td>
                            <td class="action-icons">
                                <a href="{{ route('vendor.ads.single.edit', [$mads->id, $ad->id]) }}" class="action-icon" title="Ndrysho"><i class="fa fa-edit" aria-hidden="true"></i></a>
