<x-admin-layout>
    ...
    <div>
        <div class="product-area mt-2">
            <form action="{{ route('admin.settings.footer.update', [$lid, $id]) }}" method="POST">
                @csrf
                <div class="form-group">
                    <label for="name">Emri i Linkut</label>
                    <input type="text" name="name" class="form-control" id="name" placeholder="Emri" value="{{ $link->name }}">
                    @error('name') <span class="text-danger error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label for="link">Linku</label>
                    <input type="text" name="link" class="form-control" id="link" placeholder="Link" value="{{ $link->link }}">
