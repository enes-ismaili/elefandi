<x-admin-layout>
    ...
    <div x-data="{ selOption: {{ (old('iconType') ? 'false' : 'true') }} }">
        <div class="product-area mt-2">
            <form action="{{ route('admin.settings.countries.cities.update', [$country->id, $city->id]) }}" method="POST">
                @csrf
                <div class="form-group">
                    <label for="categoryname">Emri i Qytetit</label>
                    <input type="text" name="name" class="form-control" id="categoryname" placeholder="Emri" value="{{ $city->name }}">
