<x-admin-layout>
    ...
    @endpush
    <form action="{{ route('vendor.offers.store', $offer->id) }}" method="POST" class="search-product">
        @csrf
        @livewire('offer.create-offer', ['vendor_id' => $vendor->id, 'exis'=> true, 'current'=>$offer, 'editRole'=> 0])
    </form>
</x-admin-layout>