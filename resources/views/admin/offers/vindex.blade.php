<x-admin-layout>
    ...
    <div class="product-area">
        <div class="row">
            <div class="col-12">
                <a href="{{route('vendor.offers.new')}}" class="btn btn-primary small tableadd c3" ><i class="fas fa-plus"></i> Shto Ofertë</a>
            </div>
        </div>
        <div class="row">
                    ...
                    <tbody>
                        @foreach($specialOffers as $offer)
                            ...
                            <tr>
                                ...
                                <td>{{$offer->name}}</td>
                                <td>{{ count($offer->details->where('vendor_id', '=', current_vendor()->id)) }}</td>
                                <td>{{\Carbon\Carbon::parse($offer->start_date)->format('d.m.Y')}}</td>
                                <td>{{\Carbon\Carbon::parse($offer->expire_date)->format('d.m.Y')}}</td>
                                <td class="action-icons">
                                    <a href="{{ route('vendor.offers.edit', $offer->id) }}" class="action-icon" title="Shto Produkte">Shto Produkte</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            @endif
            <div class="col-12">
                <h5 class="table-header-t">Ofertat e Mia</h5>
                <table id="myTable" class="table">
                    <thead>
                        <tr>
                            <th style="width: 75px">#</th>
                    ...
                    <tbody>
                        @foreach($offers as $offer)
                            ...
                            <tr>
                                ...
                                <td>{{\Carbon\Carbon::parse($offer->expire_date)->format('d.m.Y')}}</td>
                                <td class="action-icons">
                                    <a href="{{ route('vendor.offers.edit', $offer->id) }}" class="action-icon" title="Ndrysho"><i class="fas fa-edit" aria-hidden="true"></i></a>
                                    @if(check_permissions('delete_rights'))
                                        <span class="deleteModal action-icon"  title="Fshi" onclick="deleteModalF(this)"
                                            data-link="{{ route('vendor.offers.delete', $offer->id) }}" 
                                            data-text="Ju po fshini Ofertën '{{$offer->name}}'"
                                            data-type="Ofertë"><i class="fas fa-trash" class="action-icon"></i>
                                        </span>
                                    @endif
                                </td>
                            </tr>
