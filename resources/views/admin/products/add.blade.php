<x-admin-layout>
    @push('scripts')
        ...
        <script>
            ...
            @if(old('variant_colors'))
                window.selectedColors = ["{!! collect(old('variant_colors'))->implode('", "') !!}"]
            @else
                window.selectedColors = []
            @endif
            @if(old('variant_attribute'))
                window.selectedAttribute = ["{!! collect(old('variant_attribute'))->implode('", "') !!}"]
            @else
                window.selectedAttribute = []
            @endif
            @if(old('variant_id'))
                @php
                    $variantsArr = [];
                    $variantsArr2 = [];
                    foreach(old('variant_id') as $key=>$variant) {
                        $thisVar = [
                            'id' => $variant,
                            'name' => ((old('variant_name')[$key])?old('variant_name')[$key]:''),
                            'price' => ((old('variant_price')[$key])?old('variant_price')[$key]:''),
                            'sku' => ((old('variant_sku')[$key])?old('variant_sku')[$key]:''),
                            'qty' => ((old('variant_qty')[$key])?old('variant_qty')[$key]:''),
                            'img' => ((old('variant_img')[$key])?old('variant_img')[$key]:''),
                        ];
                        $variantsArr2['v'.$variant] = $thisVar;
...
.shipping-product.show {
    ...
    <x-slot name="breadcrumb">
        <h4 class="heading">Shto Produkt</h4>
        <ul class="links">
            <li>
            ...
            <li>
                <a href="{{route('vendor.products.index')}}">Produktet</a>
            </li>
        </ul>
    </x-slot>
        ...
        <div class="row">
            <div class="col-xl-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" class="form-control" id="productname" placeholder="Emri" value="{{ old('name') }}">
                            @error('name') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group">
                            <label for="product-description">Përshkrimi</label>
                            <textarea class="form-control" name="description" id="product-description" rows="3" placeholder="Përshkrimi">{{ old('description') }}</textarea>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="weight" class="form-control" id="product_weight" placeholder="Psh: 1 kg" value="{{ old('weight') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="size" class="form-control" id="product_size" placeholder="Psh: 10x20x30" value="{{ old('size') }}">
                            @error('size') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="row" x-data="{ openPersonalize: false }">
                            <label class="col-md-12">Mundësia e Personalizimit</label>
                            <div class="form-group col-md-12 ">
                                <label class="cth-switch cth-switch-success mb-0">
                                    <input type="checkbox" name="activepersonalize" x-on:click="openPersonalize = !openPersonalize">
                                    <span></span>
                                </label>
                            ...
                            <div class="form-group col-md-12 personalize-title" :class="{ 'show ': openPersonalize === true }">
                                <input class="form-control" type="text" name="personalizetitle" placeholder="Shkruani llojin e personalizmit që do ti shfaqet përdoruesit" value="">
                            </div>
                        </div>
                        <label for="">Shtoni më shumë specifikime</label>
                        <div id="customfield">
							@if(old('customfield_name'))
							@foreach(old('customfield_name') as $specificationName)
                            <div class="row">
                                <div class="col-5">
                                    <div class="form-group">
                                        <input class="form-control" type="text" name="customfield_name[]" placeholder="Emri i Fushes" required value="{{ $specificationName }}">
                                    </div>
                                </div>
                                <div class="col-5">
                                    <div class="form-group">
                                        <input class="form-control" type="text" name="customfield_value[]" placeholder="Vlera i Fushes" required value="{{ old('customfield_value')[$loop->index] }}">
                                    </div>
                                </div>
                                <div class="col-2">
                                    <div class="form-group">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <select id="selectColors" multiple name="variant_colors[]">
                                @foreach($colorsList as $color)
                                    <option value="{{$color->id}}" class="{{$color->slug}}" @if(old('variant_colors') && in_array($color->id, old('variant_colors'))) selected @endif>{{$color->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <label class="cth-switch cth-switch-success mb-0">
                                <input value="1" type="checkbox" name="colors_active" id="colors_active" @if(old('colors_active')) checked @endif>
                                <span></span>
                            </label>
                        </div>
                        <div class="form-attributes @if(old('colors_active')) show @endif">
                            <div class="form-group">
                            ...
                            <div class="customer_choice_options" id="customer_choice_options">
                                @if(old('variant_attributes'))
                                @ray(old('variant_attributes'))
                                @foreach (old('variant_attribute') as $key=>$attribute)
                                    @ray($attribute)
                                    <div class="form-group row variants-option-{{$attribute}}">
                                        <div class="col-md-3">
                                            <input type="hidden" name="choice_no[]" value="1">
                                            <input type="text" class="form-control" name="choice[]" value="{{ old('choice')[$key] }}" placeholder="Choice Title" readonly="">
                                        </div>
                                        <div class="col-md-8">
                                            <input type="text" id="variants-option-{{$attribute}}" class="variant-input" name="variant_attributes[{{$attribute}}]" value="{{ old('variant_attributes')[$attribute] }}" />
                                        </div>
                                    </div>
                                @endforeach
                                @endif
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="text" name="price" class="form-control" id="productprice" placeholder="Psh: 2" value="{{ old('price') }}">
                            @error('price') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group">
                            <label for="productsku">Sku</label>
                            <input type="text" name="sku" class="form-control" id="productsku" placeholder="Sku" value="{{ old('sku') }}">
                            @error('sku') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="form-group">
                            <label for="productstock">Stoku</label>
                            <input type="text" name="stock" class="form-control" id="productstock" placeholder="Psh: 5" value="{{ old('stock') }}">
                            @error('stock') <span class="text-danger error">{{ $message }}</span>@enderror
                        </div>
                        <div class="price_combination @if(old('variant_id') && count(old('variant_id'))>0) show @endif" id="price_combination">
                            <table class="table table-bordered aiz-table footable footable-8 breakpoint-xl">
                                <thead>
                                    <tr class="footable-header">
                                        <td class="tcenter footable-first-visible" style="display: table-cell;">Varianti</td>
                                        <td class="tcenter" style="display: table-cell;">Çmimi i Variantit</td>
                                        <td class="tcenter" data-breakpoints="lg" style="display: table-cell;">SKU</td>
                                        <td class="tcenter" data-breakpoints="lg" style="display: table-cell;">Stoku</td>
                                        <td class="tcenter table-variant_image" data-breakpoints="lg" style="display: table-cell;">Foto</td>
                                        <td class="footable-last-visible table-actions" style="display: table-cell;"></td>
                                    </tr>
                                </thead>
                                <tbody id="price_combination_table">
                                    @if(old('variant_id'))
                                    @foreach (old('variant_id') as $key=>$item)
										@php
											print_r(old('variant_img')[$key]);
										@endphp
                                        <tr class="variant" id="table-price-{{$item}}">
                                            <td class="footable-first-visible">
                                                <label for="" class="control-label">{{$item}}</label>
                                                <input type="hidden" name="variant_id[]" value="{{ old('variant_name')[$key] }}" class="svariant_id">
                                                <input type="hidden" name="variant_name[]" value="{{$item}}" class="svariant_name">
                                            </td>
                                            <td style="display: table-cell;width: 160px;">
                                                <div class="input-group">
                                                    <input type="number" lang="en" name="variant_price[]" value="{{ old('variant_price')[$key] }}" min="0" step="0.01" class="form-control small variant_price svariant_price" placeholder="{{ old('price') }}">
                                                    <span class="input-group-text small">€</span>
                                                </div>
                                            </td>
                                            <td style="width: 130px;">
                                                <input type="text" name="variant_sku[]" class="form-control svariant_sku" value="{{ old('variant_sku')[$key] }}">
                                            </td>
                                            <td style="width: 100px;">
                                                <input type="number" lang="en" name="variant_qty[]" value="{{ old('variant_qty')[$key] }}" min="0" step="1" class="form-control variant_qty svariant_qty" placeholder="{{ old('stock') }}">
                                            </td>
                                            <td>
                                                <div class="input-group table-variant_image @if( old('variant_img')[$key] ) upload @endif" data-toggle="aizuploader" data-type="image">
                                                    <label for="variant_image_{{ old('variant_name')[$key] }}">
                                                    ...
                                                    <div class="view_image">
                                                        @if( old('variant_img')[$key] )
                                                            <div class="remove-image" onclick="removeVariantImage(this)"><i class="fas fa-times"></i></div>
                                                            <img src="{{asset('/photos/'. old('variant_img')[$key] )}}">
                                                        @endif
                                                    </div>
                                                    <input type="file" class="variants_images" id="variant_image_{{ old('variant_name')[$key] }}" hidden>
                                                    <input type="hidden" name="variant_img[]" class="selected-files svariants_image" value="{{ old('variant_img')[$key] }}">
                                                </div>
                                            </td>
            ...
            <div class="col-xl-4 product-right">
                                ...
                                <div class="body-area">
                                    <div class="form-group">
                                        ...
                                        @livewire('upload-file', [
                                            'inputName' => 'image', 'upload' => 'single', 'exis' => ((old('image')) ? old('image') : ''), 'path'=> 'products/', 'type'=>2, 'deleteF'=>true, 
                                    ...
                                    <div class="form-group">
                                        ...
                                        @livewire('upload-file', [
                                            'inputName' => 'gallery_image', 'upload' => 'multiple', 'exis' => '', 'path'=> 'products/', 'type'=>2, 'deleteF'=>true, 
                                            'paragraphText' => 'Ngarkoni foto të tjera për këtë produkt', 'maxWidth'=>700, 'maxHeight'=>700, 'uid' => 2,
                                            'sizes' => ['550']
                                        ])
                                        @error('gallery_image') <span class="text-danger error">{{ $message }}</span>@enderror
                                    </div>
                                    @php
                                        if(old('subsubCategory')){
                                            $selCategory = old('subsubCategory');
                                        } else if(old('subCategory')){
                                            $selCategory = old('subCategory');
                                        } else if(old('parentCategory')){
                                            $selCategory = old('parentCategory');
                                        } else {
                                            $selCategory = '';
                                        }
                                    @endphp
                                    @livewire('products.product-categories', ['oldid'=>true, 'category'=>$selCategory])
                                    @error('parentCategory') <span class="text-danger error">{{ $message }}</span>@enderror
                                    <div class="form-group">
                                    ...
                                    <div class="form-group productTags">
                                        ...
                                        <input type="text" value="{{ old('tags') }}" id="producttags" name="tags" />
                                        <p class="mb-0">
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        <div class="shipping-vendor" x-data="{ open_vendor: false }">
                            <div class="col-12 row nospace">
                                <label class="col-10 col-from-label title-13">Cmimet për Posten nga Dyqani</label>
                                <input type="hidden" name="vendor_shipping" value="0">
                                <div class="col-2 nospace">
                                    <label class="cth-switch cth-switch-success mb-0">
                                        <input value="1" type="checkbox" name="vendor_shipping" x-on:click="open_vendor = !open_vendor">
                                        <span></span>
                                    </label>
                                </div>
                            </div>
