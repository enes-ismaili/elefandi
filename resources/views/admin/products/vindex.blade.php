<x-admin-layout>
    ...
    <div class="product-area">
        <div class="row">
            <div class="col-12">
                <a href="{{route('vendor.products.new')}}" class="btn btn-primary small tableadd c3" ><i class="fas fa-plus"></i> Shto Produkt</a>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="action-icons">
                                <a href="{{ route('single.product', [$product->owner->slug, $product->id]) }}" class="action-icon" title="Shiko" target="_blank"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                <a href="{{route('vendor.products.edit', $product->id)}}" class="action-icon" title="Ndrysho"><i class="fas fa-edit" aria-hidden="true"></i></a>
                                <a href="{{route('vendor.offers.newp', $product->id)}}" class="action-icon" title="Bëj ofertë"><i class="fas fa-percentage" aria-hidden="true"></i></a>
                                @if(check_permissions('delete_rights'))
                                    <span class="deleteModal action-icon"  title="Fshi" onclick="deleteModalF(this)"
                                        data-link="{{ route('vendor.products.delete', $product->id) }}" 
