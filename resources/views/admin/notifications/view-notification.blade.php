            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" class="form-control" id="title" placeholder="Titulli" value="{{ $notification->title }}" disabled>
                        @error('title') <span class="text-danger error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label for="message">Mesazhi</label>
                        <textarea class="form-control" name="message" id="message" rows="3" placeholder="Mesazhi" disabled>{{ $notification->message }}</textarea>
                    ...
                    @if($notification->ntype == 2)
                        <div class="form-group">
                            ...
                            <input type="text" name="plink" class="form-control" id="plink" placeholder="Linku i Produktit" value="{{ ($notification->ntype == 2) ? $notification->link : '' }}" disabled>
                    ...
                    @else
                        @if($notification->ntype == 3)
                            @php
                                $fields = [];
                                if($notification->nactive){
                                    $fields = unserialize($notification->fields);
                                } else {
                                    $fields['coupon'] = $notification->fields;
                                }
                            @endphp
                            <div class="form-group">
                                <label for="coupon">Kuponi *</label>
                                <input type="text" name="coupon" class="form-control" id="coupon" placeholder="Kuponi" value="{{ isset($fields['coupon']) ? $fields['coupon'] : '' }}" disabled>
                                @error('coupon') <span class="text-danger error">{{ $message }}</span>@enderror
                            </div>
                        @endif
                        <div class="form-group">
                            <label for="vlink">Linku i Dyqanit *</label>
                            <input type="text" name="vlink" class="form-control" id="vlink" placeholder="Linku i Dyqanit" value="{{ ($notification->ntype != 2) ? $notification->link : '' }}" disabled>
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <div>
                            <label class="cth-switch cth-switch-success mb-0">
                                <input value="1" type="checkbox" id="change_send" name="send_later" {{ ($sendAt) ? 'checked' : '' }} x-on:change="sendTime = !sendTime" disabled>
                                <span></span>
                            </label>
                    ...
                    <div class="form-group">
                        ...
                        <input class="flatpickr datetime tomorrow" type=text placeholder="Data e Përfundimit" name="send_at" value="{{ $sendAtTime }}" disabled>
                        @error('send_at') <span class="text-danger error">{{ $message }}</span>@enderror
                    </div>
                    <div class="form-group">
                        <label>Statusi i këtij Njoftimi</label>
                        <br>
                        @if($notification->nactive ==0)
                            <h5 class="btn btn-warning">Në pritje për aprovim</h5>
                        @elseif($notification->nactive ==2)
                            <h5 class="btn btn-danger">Njoftimi i refuzuar</h5>
                        @else
                            @if($notification->nsent || !$sendAt)
                                <h5 class="btn btn-primary">Dërguar</h5>
                            @else
                                <h5 class="btn btn-warning">Në pritje për tu dërguar</h5>
                            @endif
                        @endif
                    </div>
                </div>
