...
.cth-switch + label {
    ...
    @endpush
    <form action="{{route('vendor.notifications.store')}}" method="POST" class="">
        @csrf
        <div class="row"  x-data="{ notificationtype: {{ (old('ntype'))?old('ntype') : 1 }}, sendTime: {{ old('send_later') ? 1 : 0 }} }">
            <div class="col-xl-8">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select id="selectAction" name="ntype" value="{{ old('ntype') }}" x-on:change="notificationtype = $event.target.value">
                                <option value="1">Njoftim për Dyqanin</option>
                                <option value="2">Njoftim për Produkt</option>
                                <option value="3">Njoftim për Kupon</option>
                                <option value="5">Njoftim për Ofertë</option>
                            </select>
                        ...
                        @if(session('cerror'))
                            <span class="text-danger error">{{ session('cerror') }}
                        @endif
                    </div>
                </div>
            </div>
