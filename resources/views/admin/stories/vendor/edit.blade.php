<x-admin-layout>
    ...
    <div x-data="{ utype: {{ (($story->type) ? $story->type : 1) }} }">
        <div class="product-area mt-2">
            <form action="{{ route('vendor.stories1.update', $story->id) }}" method="POST">
                @csrf
                @if($expiredStory)
                <div class="form-group">
                    <p class="text-warning">Ky story ka përfunduar. Ju nuk mund ta ndryshoni</p>
                </div>
                @elseif($story->cactive == 3)
                <div class="form-group">
                    <p class="text-warning">Ky story është refuzuar nga administratorët. Ju nuk mund ta ndryshoni</p>
                </div>
                @elseif($story->cactive == 1)
                <div class="form-group">
                    <p class="text-warning">Ky story është pranuar nga administratorët. Ju nuk mund ta ndryshoni</p>
                </div>
                @endif
                <div class="form-group">
                    <label for="storyType">Lloji</label>
                    <select name="storyType" id="storyType" x-on:change="utype = $event.target.value" {{ !$expiredStory ? '' : 'disabled'}}>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control" id="name" placeholder="Emri i butonit" value="{{ $story->name }}" {{ !$expiredStory ? '' : 'disabled'}}>
                    @error('name') <span class="text-danger error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label for="link">Linku</label>
                    <input type="text" name="link" class="form-control" id="link" placeholder="Linku" value="{{ $story->link }}" {{ !$expiredStory ? '' : 'disabled'}}>
                    @error('link') <span class="text-danger error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label for="duration">Kohezgjatja</label>
                    <input type="number" name="duration" class="form-control" id="duration" placeholder="Kohezgjatja" value="{{ $story->length }}" min="1" max="15" {{ !$expiredStory ? '' : 'disabled'}}>
                ...
                @if(!$expiredStory && ($story->cactive != 3 && $story->cactive != 1))
                <div class="form-group">
